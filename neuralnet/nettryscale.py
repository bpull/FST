...
def read_data_sets(dtype=tf.float32):
    ...
    filename_queue = tf.train.string_input_producer(["../data/stockdata/gooddata/GOOGL/GOOGL.csv"])
...
def run_net(hl1, hl2, learnrate, optimizer):
    MSE = 1000000
    ...
    with tf.Session() as sess:
        ...
        for i in range(13):
            ...
            #     plt.plot(trainys[:30], color='blue')
            #     plt.plot(batch_ys[:30], color='black')
            #     plt.show()
        ...
        # newys = []
        # for element, estd, emean in zip(ys, std, mean):
        #     element *= estd;
        #     element += emean
        #     newys.append(element)

        # print "Done Teaching, Now graphing"
        # try:
        #     fig = plt.figure(figsize=(10,7))
        #     plt.plot(newys[:100], color='blue')
        #     plt.plot(stockInfo.test.labels[:100], color='black')
        #     plt.show()
        # except Exception as e:
        #     print str(e)
